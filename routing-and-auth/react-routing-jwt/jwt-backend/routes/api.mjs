...
router.route('/login').post(requireJson, loginUser);

router.route('/logout').get(logoutUser);

router.route('/register').post(requireJson, registerUser);

router
  .route('/products/:id([0-9a-fA-F]+)')
  .all(requireJson)
  .get(requireAuthenticated, requireAdmin,getProduct)
  .put(requireAuthenticated, requireAdmin, updateProduct)
  .delete(requireAuthenticated, requireAdmin, deleteProduct);

router
  .route('/products')
  .all(requireAuthenticated, requireJson)
  .get(getAllProducts)
